...
#---------------------------------------------------------
# A Boolean function to return True if the given
# year is a leap year, False otherwise.


# Here's a very concise solution that makes use of
# some Boolean operators in a single expression.
def leap_year(year):               # The given year is a leap year if...
    return (year % 4 == 0 and      # it's divisible by four and... 
              (year % 100 != 0 or  # not divisible by one hundred or...
               year % 400 == 0))   # it's divisible by one hundred and four hundred


# Doing it the long way...
# The following solution does exactly the same thing as the
# one above but uses explicit conditional statements.  Test
# it to make sure!  Some people may find it easier to
# understand this version because it's more "operational"
# but it clearly lacks the elegance of the preferred solution
# above.
def ALT_leap_year(year):
    if year % 4 == 0:
        if year % 100 == 0:
            if year % 400 == 0: 
