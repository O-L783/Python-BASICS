...
# A recursive function to find the number of digits 
# in a decimal integer number
#
def num_digits(number):

    if number < 0: # Recursive case 1: Negative number
        return num_digits(-number) # Change the sign
    
    elif number < 10: # Base case: Just one digit
        return 1

    else: # Recursive case 2: More than one digit
